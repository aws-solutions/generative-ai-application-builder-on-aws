...
def handler(event, context):
    try:
        ...
        builder_metrics = BuilderMetrics(
            USE_CASE_UUID, os.environ["SOLUTION_ID"], os.environ["SOLUTION_VERSION"], metric_data, account_id
        )
        push_builder_metrics(builder_metrics)
    except Exception as ex:
        logger.error(f"Error occurred when sending cloudwatch metrics, Error is {ex}")
