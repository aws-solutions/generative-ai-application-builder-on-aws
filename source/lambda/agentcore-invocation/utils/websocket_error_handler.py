...
from utils.websocket_gone_exception import is_gone_exception
    ...
    def post_token_to_connection(self, payload) -> None:
        ...
        Raises:
            Exception: if there is a non-GoneException error posting the payload to the connection.
                       GoneException is suppressed (connection is already dead).
        """
        try:
            ...
            if is_gone_exception(e):
                ...
                return
            logger.error(f"Error sending token to connection {self.connection_id}: {e}", xray_trace_id=os.environ.get(TRACE_ID_ENV_VAR))
            raise e
