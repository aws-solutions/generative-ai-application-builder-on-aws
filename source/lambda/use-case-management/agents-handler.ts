...
import { checkEnv, handleLambdaError, getRootResourceId, parseEventBody } from './utils/utils';
...
import { AGENT_CORE_DEPLOYMENT_REQUIRED_ENV_VARS, UseCaseTypeFromApiEvent, Status } from './utils/constants';
...
commands.set('list', new ListAgentBuilderCommand());
commands.set('get', new GetUseCaseCommand());

const routeMap = new Map([
    ['GET:/deployments/agents', 'list'],
    ['POST:/deployments/agents', 'create'],
    ['GET:/deployments/agents/{useCaseId}', 'get'],
    ['PATCH:/deployments/agents/{useCaseId}', 'update'],
    ['DELETE:/deployments/agents/{useCaseId}', 'delete']
]);
...
export const agentsLambdaHandler = async (event: APIGatewayEvent) => {
    ...
    try {
        ...
        const agentAction = event.httpMethod && event.resource ? `${event.httpMethod}:${event.resource}` : 'unknown';
        return handleLambdaError(error, agentAction, 'Agent');
    }
};
...
export const adaptEvent = async (
    ...
    if (stackAction === 'list') {
        ...
        return new AgentBuilderUseCaseInfoAdapter(event);
    ...
    if (useCaseType !== UseCaseTypeFromApiEvent.AGENT_BUILDER) {
        throw new Error(`Unsupported UseCaseType: ${useCaseType}`);
    }
    return new AgentBuilderUseCaseDeploymentAdapter(event, rootResourceId);
};

export const agentsHandler = middy(agentsLambdaHandler).use([
