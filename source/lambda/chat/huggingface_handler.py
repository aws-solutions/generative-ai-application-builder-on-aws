...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict:
    """Create a Hugging Face RAG or non-RAG LLM object based on the configuration in `event` and
    ...
    try:
        ...
        source_docs_formatter = (
            huggingface_client.builder.knowledge_base.source_docs_formatter
            if huggingface_client.builder.knowledge_base
            else None
        )
        ...
        logger.error(f"An exception occurred in the processing of Hugging Face chat: {ex}", xray_trace_id=tracer_id)
