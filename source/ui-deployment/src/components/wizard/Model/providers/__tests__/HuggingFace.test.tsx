...
import { HF_MODEL_OPTION_IDX, MODEL_FAMILY_PROVIDER_OPTIONS } from '@/components/wizard/steps-config';
import { HuggingFaceModel } from '../HuggingFace';
...
describe('HuggingFace', () => {
    test('renders', () => {
        ...
        const mockModelData = {
            modelProvider: MODEL_FAMILY_PROVIDER_OPTIONS[HF_MODEL_OPTION_IDX]
        };

        const { cloudscapeWrapper } = renderWithProvider(
            <HuggingFaceModel {...mockFormComponentCallbacks()} modelData={mockModelData} />,
            { route: '/wizardView' }
        );
