...
# Script to run unit tests for the Workflow Agent
...
check_uv_available() {
    ...
    if [ "$uv_version" = "unknown" ]; then
        log_warning "UV found but version could not be determined"
        return 1
    fi
    
    log_info "UV detected (version: $uv_version)"
    return 0
}

show_uv_install_instructions() {
    log_error "UV is not installed. This project requires UV for dependency management."
    log_info ""
    log_info "  1. Using pip (recommended for corporate environments):"
    log_info "     pip install uv>=0.5.0"
    log_info ""
    log_info "  2. Using pipx (isolated installation):"
    log_info "     pipx install uv>=0.5.0"
    log_info ""
    log_info "  3. Using your system package manager:"
    log_info "     - macOS: brew install uv"
    log_info "     - Linux: Check your distribution's package manager"
    log_info ""
    log_info "  4. For more installation options, visit:"
...
setup_uv_environment() {
    ...
    if [ ! -f "pyproject.toml" ]; then
        log_warning "pyproject.toml not found, falling back to pip workflow"
    ...
    if [ -d "$common_lib_path" ]; then
        ...
        log_warning "gaab-strands-common directory not found at $common_lib_path"
    fi
...
run_tests() {
    ...
    # Run pytest with enhanced error handling
    if [ "$COVERAGE" = "true" ]; then
        log_info "Running tests with coverage..."
        if ! $python_exec -m pytest test/ --cov --cov-report=term-missing --cov-report=xml -v; then
        ...
        if [ "${COVERAGE_HTML:-false}" = "true" ]; then
            log_info "HTML coverage report available in htmlcov/"
        fi
        
    else
        log_info "Running tests..."
        if ! $python_exec -m pytest test/ -v; then
            log_error "Unit tests failed!"
            log_error ""
...
display_usage() {
    ...
    echo "  PYTHON_CMD=python3.13 ./scripts/run_unit_tests.sh"
    echo ""
    echo "  # Skip virtual environment (use system Python with UV)"
    ...
    echo ""
    echo "  # Run tests directly with UV (no script)"
...
# Main execution
main() {
    echo "🧪 Running unit tests for Workflow Agent..."
    echo "================================================"
    validate_python
    validate_test_environment
    setup_virtual_environment
