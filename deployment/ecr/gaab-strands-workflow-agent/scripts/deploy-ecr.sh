...
echo "=== Deploying Workflow Agent to ECR ==="
...
if [ ! -f "Dockerfile" ]; then
    ...
    log_error "This script must be run from deployment/ecr/gaab-strands-workflow-agent/scripts/"
    exit 1
fi
...
ECR_REPOSITORY="${ECR_REPOSITORY:-gaab-strands-workflow-agent}"
...
deployment_summary() {
    ...
    echo "     export WORKFLOW_AGENT_IMAGE_URI=\"$ECR_URI:$IMAGE_TAG\""
    echo ""
    ...
    echo "  4. Test the workflow agent with multiple specialized agents"
    echo ""
}
